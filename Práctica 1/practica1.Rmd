...
downsampling<-function(datos) {
  ...
  set.seed(32)
  datos_finales<-downSample(datos[, -which(names(datos) %in% c("isFraud"))], etiquetas, yname='isFraud')
...
# Volvemos a eliminar las nuevas columnas que han surgido con desviación estándar=0 tras quitar valores perdidos
minitrain_m1<-eliminar_columnas_sd(minitrain_m1, FALSE)
...
Como podemos observar en los resultados este modelo ha obtenido aproximadamente un **77% de precisión** que si bien en cifra no está mal, observando la matriz de confusión podemos comprobar que se equivoca 1/3 de las veces puesto que el conjunto de validación solo contaba con 161 muestras. Por lo tanto, podemos determinar que la **reducción tan brusca de los datos** que se ha realizado ha mermado bastante la capacidad de entrenar un clasificador competente así como poder validarlo con un conjunto de datos suficientemente amplio como para tener más en cuenta estos resultados. 
...
Tal y como podemos observar este modelo ha obtenido aproximadamente un **84% de precisión**, 7 puntos más que el anterior modelo. Si bien ambos han sido entrenados y validados usando los mismos conjuntos de datos, en este modelo se puede apreciar una mayor robustez frente a la escasez de datos de entrenamiento puesto que los resultados son notablemente mejores.
...
[] Documentación acerca de la función createDataPartition, https://www.rdocumentation.org/packages/caret/versions/6.0-85/topics/createDataPartition
[] Documentación de la función confusionMatrix, https://www.rdocumentation.org/packages/caret/versions/3.45/topics/confusionMatrix


------------------------------------------------------------------------------------------------

FactoMineR users, MissMDA error message, https://groups.google.com/forum/#!topic/factominer-users/VlAXWYwSpDw

RPubs, Joaquín Amat Rodrigo, Análisis de Componentes Principales (Principal Component Analysis, PCA) y t-SNE, https://rpubs.com/Joaquin_AR/287787

Documentación acerca de la función prcomp, https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/prcomp

 R-bloggers, thiagogm, Near-zero variance predictors. Should we remove them?, https://www.r-bloggers.com/near-zero-variance-predictors-should-we-remove-them/
