...
En primer lugar vamos a reducir el número de transacciones mediante la función `sample` [2] de modo que seleccionaremos hasta 50.000 muestras aleatoriamente incluyendo todas las columnas del dataset. Esta operación es necesaria realizarla previa al procesamiento de la información puesto que es imposible aplicarle las siguientes técnicas en un tiempo razonable.
...
Analizando más en profundidad la información que nos aportan las columnas desde la propia página del dataset [1] me he percatado de que las variables **C1-C14 están cifradas**, lo cual quiere decir que si no disponemos de la clave de encriptado no podemos acceder a la información real, y por tanto, disponemos de 14 columnas con información que no significa nada. Por lo tanto, como segundo paso de este preprocesamiento se desarrolla la siguiente función que se encarga de eliminar dichas columnas dado un dataset.
...
Para realizar este preprocesamiento vamos a hacer uso de la función `nearZeroVar` [7] que calcula el número de valores únicos y su frecuencia de aparición. De este modo obtendremos aquellas variables cuya desviación estándar sea 0 o cercana a dicho valor. Previo a su aplicación, es necesario transformar todas las variables a variables numéricas para poder calcular la desviación estándar. Para las columnas categóricas se asignará a cada etiqueta un único valor que la represente.
...
* **Imputación de NAs**. Al contrario que la anterior, esta técnica más sofisticada es una de las más utilizadas según he estado investigando. Para aplicarla existen varias librerías pero al parecer la más popular es `mice` debido a las múltiples operaciones que puede llevar a cabo para asignar un valor a cada NA [8]. Dependiendo de la naturaleza de los datos, existen métodos específicos aunque también dispone de métodos generales aplicables a cualquier tipo de dato.
...
A continuación comprobamos el número de variables resultante tras aplicar este primer proceso. Tal y como podemos observar hemos pasado de **394 variables a 221**, que si bien siguen siendo muchas solo eliminando las columnas con más de un 50% de NAs hemos conseguido una buena reducción del número de columnas.

### Variables irrelevantes
La primera lista con dos columnas son aquellas variables cuya desviación estándar es **igual a 0**. Esto lo podemos comprobar fácilmente observando el porcentaje de valores únicos que también es 0. Estudiando más a fondo sendas variables he podido observar que `M1` cuenta con valores perdidos y unos, mientras que los valores de `V305` son unos casi en su totalidad. Por lo tanto procedemos a eliminarlas del conjunto de datos.

La siguiente lista con variables muestra aquellas columnas con una desviación estándar **cercana a 0**. Pero ¿nos conviene eliminarlas todas, obviando la variable a predecir? Según este artículo [8] las variables con desviación estándar cercana a 0 pueden sernos de utilidad y no conviene eliminarlos todos de una vez. Asimismo, también he experimentado este problema en la asignatura Aprendizaje Automático donde los clasificadores aportaban mejores resultados no eliminando este tipo de variables. Por lo que por el momento, seguirán estando en el conjunto de datos.
...
[7] Documentación sobre la función nearZeroVar, https://www.rdocumentation.org/packages/caret/versions/6.0-85/topics/nearZeroVar
[8] Documentación sobre la función mice, https://www.rdocumentation.org/packages/mice/versions/2.25/topics/mice
[9] Documentación acerca de la función findCorrelation, https://rdrr.io/rforge/caret/man/findCorrelation.html
[10] r-statistics.co, Selva Prabhakaran, Outlier Treatment, http://r-statistics.co/Outlier-Treatment-With-R.html
[11] Documentación sobre la función boxplot, https://www.rdocumentation.org/packages/grDevices/versions/3.6.2/topics/boxplot.stats
